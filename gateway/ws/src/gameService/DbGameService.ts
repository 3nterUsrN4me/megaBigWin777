...
export class DbGameService {
  ...
  async placeBet(params: {
    ...
    if (ps.bet !== null) {
      ...
      return {
        ok: true, roundStarted: false, game: null, slotIndex: resolvedSlotIndex!,
        playerChips, roomState: await this.buildRoomState(room), minBet: TABLE_MIN_BET, maxBet: TABLE_MAX_BET,
      };
    }
    ...
    if (!allBetsIn) {
      return {
        ok: true, roundStarted: false, game: null, slotIndex: resolvedSlotIndex!,
        playerChips: reserved.chips, roomState: await this.buildRoomState(room),
        minBet: TABLE_MIN_BET, maxBet: TABLE_MAX_BET,
      };
    }

    const seatBets: SeatBet[] = Array.from(room.slots.entries()).map(([seatIndex, entry]) => ({
      seatIndex,
      playerId: entry.playerState.playerId,
      betAmount: entry.playerState.bet!,
    }));

    const dealResult = await this.gameService.startMultiplayerRound(tableId, seatBets);
    if (!dealResult.ok) {
    ...
    for (const [seatIndex, view] of dealResult.games) {
      ...
      if (entry) {
        ...
        if (seatIndex === resolvedSlotIndex) actorGame = view;
      }
    }
    ...
    const playerChips = await this.getPlayerChips(playerId);
    return {
      ok: true, roundStarted: true, game: actorGame, slotIndex: resolvedSlotIndex!,
      playerChips, roomState: await this.buildRoomState(room, true),
      minBet: TABLE_MIN_BET, maxBet: TABLE_MAX_BET,
    };
  }
